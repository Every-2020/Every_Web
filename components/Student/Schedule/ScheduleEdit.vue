<template>
  <div class="schedule_edit">
    <div class="schedule_edit_area" @click="close" />
    <div class="schedule_edit_box">
      <div class="schedule_edit_title">
        <span>일정 제목</span>
      </div>
      <input v-model="title" class="schedule_edit_title_input" type="text" placeholder="일정 제목을 입력해주세요." maxlength="20">
      <div class="schedule_edit_title">
        <span>일정 내용</span>
      </div>
      <input v-model="content" class="schedule_edit_title_input" type="text" placeholder="일정에 관한 세부적인 내용을 입력해주세요." maxlength="150">
      <div class="schedule_edit_title">
        <span>일정 시작 날짜</span>
      </div>
      ...
      <div class="schedule_btn">
        <div class="schedule_btn_com" @click="edit">
          수정
        </div>
  ...
  methods: {
    edit () {
      if (this.title && this.content && this.start <= this.end) {
        axios.put(`${this.$store.state.url}/schedule/${this.idx}`, {
          title: this.title,
